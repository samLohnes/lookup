...
def _pass(norad_id: int, rise_time: datetime, peak_az: float) -> Pass:
    return Pass(
        ...
        rise=PassEndpoint(time=rise_time, position=AngularPosition(peak_az - 90.0, 5.0), range_km=2000.0),
        peak=PassEndpoint(time=rise_time + timedelta(minutes=3), position=AngularPosition(peak_az, 60.0), range_km=600.0),
        set=PassEndpoint(time=rise_time + timedelta(minutes=6), position=AngularPosition(peak_az + 90.0, 5.0), range_km=2000.0),
        duration_s=360,
        max_magnitude=4.0,
        sunlit_fraction=1.0,
        tle_epoch=rise_time,
